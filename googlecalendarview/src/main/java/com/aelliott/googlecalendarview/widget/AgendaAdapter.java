...
public abstract class AgendaAdapter extends RecyclerView.Adapter<AgendaAdapter.RowViewHolder>
{
    ...
    public AgendaAdapter(Context context)
    {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(
                new int[] {android.R.attr.textColorTertiary});
        ...
        switch (viewType)
        {
            case VIEW_TYPE_HEADER:
                return new GroupViewHolder(
                        mInflater.inflate(R.layout.list_item_header, parent, false));
            case VIEW_TYPE_CONTENT:
            default:
                return new ContentViewHolder(
                        mInflater.inflate(R.layout.list_item_content, parent, false));
        }
    }
    ...
    {
        ArrayList<EventGroup> savedGroups = savedState.getParcelableArrayList(STATE_EVENT_GROUPS);
        ...
        switch (eventItem.mDisplayType)
        {
            ...
            default:
                contentHolder.textViewTime.setText(
                        CalendarUtils.toTimeString(context, eventItem.mStartTimeMillis));
                break;
            case EventItem.DISPLAY_TYPE_END_TIME:
                String endTimeString = CalendarUtils.toTimeString(context,
                        eventItem.mEndTimeMillis);
                contentHolder.textViewTime.setText(
    ...
    {
        int lastCount = group.mLastCursorCount, newCount = group.mCursor.getCount(), refreshCount = Math
                .min(newCount, lastCount), diff = newCount - lastCount;
    ...
    {
        EventEditView.Event.Builder eventBuilder = new EventEditView.Event.Builder().start(
                eventItem.mStartTimeMillis).end(eventItem.mEndTimeMillis).allDay(
                eventItem.mIsAllDay);
        if (!(eventItem instanceof NoEventItem))
        {
            eventBuilder.id(eventItem.mId).calendarId(eventItem.mCalendarId).title(
                    eventItem.mTitle);
        }

        // TODO Remove this?
        /*context.startActivity(
                new Intent(context, EditActivity.class).putExtra(EditActivity.EXTRA_EVENT,
                        eventBuilder.build()));*/
    }
