package com.aelliott.googlecalendarview.content;
...
public abstract class EventsQueryHandler extends AsyncQueryHandler
{
    ...
    private static final String START_WITHIN = "(" + CalendarContract.Events.DTSTART + ">=?" + AND + CalendarContract.Events.DTSTART + "<?" + ")";
    // select events that starts before but end within or after query range
    private static final String START_BEF_END_WITHIN_AFTER = "(" + CalendarContract.Events.DTSTART + "<?" + AND + CalendarContract.Events.DTEND + ">?" + ")";
    // select non all-day events
    private static final String SELECTION_NON_ALL_DAY_EVENTS = "(" + ALL_DAY + AND + "(" + START_WITHIN + OR + START_BEF_END_WITHIN_AFTER + ")" + ")";
    // select all-day events
    private static final String SELECTION_ALL_DAY_EVENTS = "(" + ALL_DAY + AND + "(" + START_WITHIN + OR + START_BEF_END_WITHIN_AFTER + ")" + ")";
    // select non-deleted events from either set
    private static final String SELECTION = "(" + DELETED + AND + "(" + SELECTION_NON_ALL_DAY_EVENTS + OR + SELECTION_ALL_DAY_EVENTS + ")" + ")";
     ...
     */
    public EventsQueryHandler(ContentResolver cr, @NonNull Collection<String> excludedCalendarIds)
    {
        super(cr);
    ...
    {
        final String utcStart = String.valueOf(
                CalendarUtils.toUtcTimeZone(startTimeMillis)), utcEnd = String.valueOf(
                CalendarUtils.toUtcTimeZone(endTimeMillis)), localStart = String.valueOf(
                startTimeMillis), localEnd = String.valueOf(endTimeMillis);
        ...
        startQuery(0, cookie, CalendarContract.Events.CONTENT_URI, EventCursor.PROJECTION,
                sb.toString(), args.toArray(new String[args.size()]), SORT);
    }
